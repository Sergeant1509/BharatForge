...
import { extractFieldReportFromImage } from "../services/aiExtractor.js";
        ...
        try {
            if (!req.file) {
                return res.status(400).json({
                    success: false,
                    message: "DPR image is required",
                });
            }

            console.log(
                `Processing DPR image: ${req.file.originalname}`
            );
            ...
            const extracted =
                await extractFieldReportFromImage(
                    req.file.buffer,
                    req.file.mimetype
                );
            ...
            if (!extracted.projectCode) {
                return res.status(400).json({
                    success: false,
                    message: "Project code could not be extracted",
                    extracted,
                });
            }
            ...
            if (!extracted.activityDescription) {
                return res.status(400).json({
                    success: false,
                    message: "Activity description could not be extracted",
                    extracted,
                });
            }
            ...
            const matching = await matchActivity({
                ...
                description: extracted.activityDescription,
            });

            console.log("AI MATCHING RESULT:", matching);
            ...
            if (
                ...
                activityId = matching.activity.id;
            }
            ...
            const reportCode = `FR-AI-${Date.now()}`;
            ...
            const reportResult = await client.query(
                ...
                [
                    ...
                    project.id,
                    activityId,
                    extracted.reportDate || null,
                    extracted.activityDescription,
                    extracted.reportedProgress ?? null,
                    extracted.status || "REPORTED",
                    extracted.discipline || null,
            ...
            return res.json({
                ...
                report: {
                    id: savedReport.id,
                    reportCode: savedReport.report_code,
                    matchingStatus:
            ...
            console.error(
                "Image ingestion error:",
                error
            );

            return res.status(500).json({
                success: false,
                message: "Failed to process DPR image",
                error: error.message,
            });
